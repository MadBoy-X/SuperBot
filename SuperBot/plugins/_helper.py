...
async def cmd_list(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/" , "#", "-", "_", "@"):
        ...
        if tgbotusername is None or input_str == "text":
            ...
            help_string = f"""ֆʊքɛʀɮօȶ Helper.. Provided by ✨{DEFAULTUSER}✨ \n
`ֆʊքɛʀɮօȶ Helper to reveal all the commands`\n__Do .help plugin_name for commands, in case popup doesn't appear.__"""
...
async def _(event):
    ...
    await event.edit("Telethon UserBot powered by SuperBotOT")
