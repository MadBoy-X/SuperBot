...
from SuperBot import CMD_LIST, SUDO_LIST
import re
import logging
import inspect
import math
...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        from pathlib import Path
        path = Path(f"SuperBot/plugins/{shortname}.py")
        name = "SuperBot.plugins.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = SuperBot.utils
        mod.Config = Config
        mod.borg = bot
        # support for paperplaneextended
        sys.modules["SuperBot.events"] = SuperBot.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["SuperBot.plugins."+shortname] = mod
        print("Successfully imported "+shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"SuperBot.plugins.{shortname}"
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                superreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                superreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(superreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                superreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def edit_or_reply(
    ...
    if aslink:
        linktext = linktext or "Message exceeded word limits, So pasted it to Bin."
        try:
            key = (
                requests.post(
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            ftext += "--------BEGIN SUPERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"SuperBot/plugins/assistant/{shortname}.py")
        name = "SuperBot.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Starting Your SuperBot Assistant.")
        print("SuperBot Assistant Initiated Successfully " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
