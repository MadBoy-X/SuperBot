...
# ported to SuperBot by madboy482
...
async def get_users(event):   
    ...
    madboy = await get_chatinfo(event) ; chat = await event.get_chat()
    if event.is_private:
              return await hell.edit("`Sorry, Can't add users here`")    
    s = 0 ; f = 0 ; error = 'None'   
  
    await hell.edit("**Terminal Status**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(madboy.full_chat.id):
                try:
