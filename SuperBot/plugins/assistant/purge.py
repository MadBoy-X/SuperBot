...
from SuperBot.utils import bot
...
@assistant.on(events.NewMessage(pattern="^/purge"))
async def purge(event):
    ...
    try:
        ...
        await assistant.delete_messages(chat, event.message.id)
        msgs.append(event.reply_to_msg_id)
        for m_id in range(to_delete, msg_id - 1, -1):
            msgs.append(m_id)
            count += 1
            if len(msgs) == 100:
                await assistant.delete_messages(chat, msgs)
                msgs = []

        await assistant.delete_messages(chat, msgs)
        del_res = await assistant.send_message(
...
@assistant.on(events.NewMessage(pattern="^/del$"))
async def delete_msg(event):
    ...
    rm = [msg, to_delete]
    await assistant.delete_messages(chat, rm)
