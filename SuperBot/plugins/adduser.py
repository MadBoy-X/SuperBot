...
async def _(event):
    ...
    if event.is_private:
        ...
        if not event.is_channel and event.is_group:
            ...
            await event.edit("Added Successfully.Now arrange a grand party in this group🥳")
        else:
            for user_id in to_add_users.split(" "):
                try:
                    ...
                    await event.reply(str(e))
            await event.edit("ADDED the user to the chat successfully. Now gib hard party🥳")
CMD_HELP.update(
    {
        "adduser": 
    ".edd <username>"
